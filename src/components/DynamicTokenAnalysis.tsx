...
export const DynamicTokenAnalysis: React.FC<TokenAnalysisProps> = React.memo(
  ({
    ...
    useEffect(() => {
      let isMounted = true;
      if (selectedToken) {
        fetchCryptoData(selectedToken.value, timeframe).then((data) => {
    ...
    const displayPriceChange =
      tokenInfo?.price_change_percentage_24h || priceChange;
      ...
      <TooltipProvider>
        <div className="space-y-4">
          ...
          <TokenAnalysisRow
            ...
            tooltipText={
              tokenInfo
                ? "Real-time market price from CoinGecko API"
          ...
          <TokenAnalysisRow
            ...
            valueClassName={`font-bold text-lg ${
              analysis.momentum > 0 ? "text-green-400" : "text-red-400"
            }`}
          />
          ...
          <div className="flex items-center gap-2 mt-4">
            ...
            >
              {displayPriceChange >= 0 ? "+" : ""}
              {displayPriceChange.toFixed(2)}% (24h)
            </span>
            <Badge
              className={`ml-auto ${
