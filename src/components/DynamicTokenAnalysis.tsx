...
import { TokenAnalysisHeader } from './TokenAnalysisHeader';
...
export const DynamicTokenAnalysis: React.FC<TokenAnalysisProps> = React.memo(({
  ...
  if (tokenInfoLoading && !selectedToken) {
    return (
      <Card className="bg-gradient-to-br from-gray-800 to-gray-900 border-gray-600 shadow-2xl">
        <TokenAnalysisHeader selectedToken={undefined} tokenInfoError={null} />
        <CardContent>
  ...
  return (
    <TooltipProvider>
      <Card className="bg-gradient-to-br from-gray-800 to-gray-900 border-gray-600 shadow-2xl">
        <TokenAnalysisHeader selectedToken={selectedToken} tokenInfoError={tokenInfoError} />
        <CardContent>
          <div className="space-y-4">
            ...
            <TokenAnalysisRow
              label="Support Level"
              value={`$${analysis.supportLevel.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 6 })}`}
              tooltipText="Key support price level based on technical analysis"
            />

            <TokenAnalysisRow
              label="Resistance Level"
              value={`$${analysis.resistanceLevel.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 6 })}`}
